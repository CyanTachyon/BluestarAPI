...
import me.lanzhi.api.command.cmdinfo.Args;
import me.lanzhi.api.command.cmdinfo.Cmd;
import me.lanzhi.api.command.cmdinfo.Label;
import me.lanzhi.api.command.cmdinfo.Sender;
...
class ArgsMap {
    ...
    Object run(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) throws Throwable {
        return run(sender, command, label, args, 0);
    }

    Object run(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args, int index) throws Throwable {
        if (args.length == index) {
            try {
                if (res != null)
                    return tryRun(sender, command, label, args, res, resParams);
                else
                    throw new Throwable("Can't match command");
            } catch (Throwable e) {
                if (any != null)
    ...
    Object tryRun(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args,
                  @NotNull Run run, @NotNull String[] format) throws Throwable {
        ...
        for (int i = 0; i < Math.min(args.length, format.length); i++) {
            ...
            else if (!format[i].equals(args[i]))
                throw new Throwable("Can't match args");
        }
        if (run.permission != null && !sender.hasPermission(run.permission))
            throw new Throwable("The sender doesn't has permission");
        if (run.only != null && run.only.length != 0) {
            ...
            if (!flag)
                throw new Throwable("The sender type does not meet the requirements");
        }
